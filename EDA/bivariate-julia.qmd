...
The data storage in `iris` is tidy data, each row is an observation, the `PetalWidth` is the variable. The graphics are done by splitting and then applying the graphic to the different groups of data. For this task, the `StatsPlots` package is useful.^[For the `Makie` plotting package, the `AlgebraOfGraphics` package is a powerful alternative to `StatsPlots` and reminiscent of R's `ggplot` system. We will illustrate its use in a later chapter.]
...
```{julia}
#| label: fig-simple-dotplot
#| fig-cap: Simple dotplot with no separation by species
xs = iris.PetalWidth
...
```{julia}
#| label: fig-dotplot-split-by-hand
#| fig-cap: A dot plot with each species presented a different $y$ value
...
```{julia}
#| echo: false
#| label: fig-dotplot-both
#| fig-cap: The left graphic shows a dot plot with groups split by species presented horizonatally. The right one a dot plot with groups split by species presented verticall. Jittering is used by defaut to disambiguate ties. The $x$ axis represents categories, it is not a numeric scale.
...
```{julia}
#| label: fig-boxplot-with-dotplot
#| fig-cap: Boxplots of `PetalWidth` displayed vertically, one for each species level.
...
```{julia}
#| label: fig-violinplot-with-dotplot
#| fig-cap: Violin plots of `PetalWidth` displayed vertically, one for each species level.
...
#| fig-cap: Illustration of boxplot (with dotplot overlay) and violin plot.
plot(p1, p2, layout=(@layout [a b]))
```
...
The box plot is a great graphic for comparing values across the levels of some factor. Each boxplot shows the center (the median), the range (the IQR), the shape (well basic symmetry about the median), the outlying values (with its identying rule of 1.5 IQRs beyond $Q_1$ and $Q_3$. The only drawback is boxplots can't show multi modal data. But if the factor is the cause of that, then the collection of boxplots is great for comparison, as in @fig-boxplot-violinplot.
...
#| fig-cap: Two lines as possible fits to the data with the sum of squared residuals reported.
x = [1,2,3,4,5]; xbar = 3
y = [2,1,4,3,5]; ybar = 3
...
Let $f(x)$ be some function, then a measure of the distance of the data to the line might be the sum $\sum_i f(e_i)$. For exmaple, using $f(x) = |x|$, we have:
...
Algebraically, the above yields these values for the $\beta$s in terms of the data, $(x_i, y_i)$:
...
The formula specifies a pattern in a certain manner, the additional
arguments indicate how the pattern refers to the data. We *could* use
a simple named tuple here, but a data frame is typical, as used here:

```{julia}
lm(@formula(PetalWidth ~ PetalLength), subset(iris, :Species => ByRow(==("setosa"))))
```
...
loess_line!(p2, l′, w′; linewidth=5, color=:red)
...
The formula for the slope of the simple regression coefficient depends on $s_y/s_x$. The standard deviation may be influenced greatly by 1 outlying value. That is, they are not very resistant to outliers. A theory or *Robust Linear Models* has been developed which are useful in allowing a relaxation on the standard distributional assumptions, relaxations that accommodate data with more outliers. The `RobustModels` package implements a number of such models, of which we illustrate just `MEstimator{TukeyLoss}` without comment as to the implemented algorithm.

One way to view data sets that benefit from robust models is to envision a process where the measured response values are given by the value on a regression line *plus* some "normally" distributed error **plus** some "contaminated" error which interoduces outliers.
...
#| fig-cap: The 3 data points in the upper right of this graphic are "contaminated" data. Their outlying nature has unde influence on the least-squares regression line ("lm") pulling it up towards the points. The robust regression line ("rlm") is not so influenced.
...
The syntax for `predict` may have seemed more complicated than needed, but the model may have many covariates, so the specification needs a means to identify them. A `Tables` compatible specification of the data used in the prediction is used, in the above that was a named tuple.^[A named tuple of vectors to specify a data set uses a struct-of-arrays; a vector of named tuples uses an array-of-structs approach.]
...
For the simple linear regression model, the value $\sum e_i^2$ is minimized by the choice of the estimates for the $\beta$s. This value is called the SSE, or sum of squared errors. It can be found from the residuals *or* the generic method `deviance`:
...
First, suppose we simply adjust the fitted lines up or down for each cluster. That is, there is some additive effect for each type of species. As a example of something we discuss at length later, we can fit this model by including `Species` as an additive term:
