...
question. This can be done by grouping and applying the `nrow` function from `DataFrames`:
...
Another common graphic is to use a dot, not a bar, to represent the value. Still another are pie charts, which are common in the business world, but not favorites within introductory statistics.
