...
A quantile-quantile graphic is created with the `QQPlot` visual. `QQPlot` expects mappings to `:x` and `:y` (the first two positions).

The `Makie` command `qqplot` can be used along with its specialization to plotting the `y` position against a normal distribution through `qqnorm`.

The following will give a visual check if bill length has a similarly *shaped* distribution as bill depth

```{julia}
p1 = data(penguins) * visual(QQPlot, qqline=:fit) *
    mapping(:bill_length_mm, :bill_depth_mm);
```

The `QQPlot` command had a regression. Before, it was possible to specify a `color` attribute to `mapping`; that now errors. As well, the `QQNorm` specialization (where the `x` is mapped to the `Normal(0,1)` distribution) currently fails. The underlying `qqnorm` function is available, though it requires some modification to specifying the data.
...
```{julia}
p2 = qqnorm(penguins.bill_length_mm, qqline=:fit);
```
...
Quantile-quantile plots. The left graphic shows `QQPlot` used to compare the distribution of two variables. The right graphic, using `qqnorm` uses a reference normal distribution for comparison.
:::
