...
This is only a space saving measure and wouldn't normally be done when interacting with `Julia`.^[For some case, where `show` takes too much horizontal space, `permutedims` is used for a similar purpose, as may elide some values.]
...
be seen through exmaple, but are widely used to represent mathematical
...
recycling involves some rules where the space rferenced on the
...
the left and then does the assignment. For example, if we wanted
...
The use of multiple shapes allows scalars and vectors to be broadcast over and is used in `whale .- mean(whale)`. But vector and *row* vectors can also be broadcast over, So `[1,2] .+ [3,4]` does vector addition, `[1,2] .+ [3 4]` pads out the vector to a matrix, the row vector to a matrix, then adds entry-by-entry:
...
(We use a semicolon to separate positional arguments from keyword arguments, as that is needed to define a keyword, but commas can be used to call a keyword argument. What is important is the keywords come last.)
